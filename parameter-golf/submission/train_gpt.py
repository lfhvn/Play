...
class MLP(nn.Module):
    def __init__(self, dim: int, mlp_mult: int, leaky_relu_slope: float = 0.75):
        super().__init__()
        self.leaky_relu_slope = leaky_relu_slope
        # No CastedLinear -- weights come from banks
    def forward(self, x: Tensor, up_w: Tensor, down_w: Tensor) -> Tensor:
        x = F.leaky_relu(F.linear(x, up_w.to(x.dtype)), negative_slope=self.leaky_relu_slope)
